...
def change_environment(exec_path: str | Path) -> None:
    """
...
def typecheck(value: Any, datatype: Any) -> bool:
    ...
    if len(anno := get_args(datatype)) > 0:
        ...
        if get_origin(datatype) in (tuple, list, dict):
...
def extract_parent_type(owner: Any, name: str) -> Any:
    """
    ...
    """
    # __annotations__ does not include base class
    ...
    annotations = functools.reduce(
        ...
        (
            ...
            for base in owner.__class__.__bases__
        ),
    )
