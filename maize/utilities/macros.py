...
from maize.core.interface import Input, MultiInput, Output, Parameter
...
def parallel(
    ...
    **kwargs: Any,
    ...
    """
    input_names = inputs or ["inp"]
    output_names = outputs or ["out"]
    ...
    def build(self: Graph) -> None:
        ...
        constant_sowers = [self.add(Copy[Any], name=f"sow-const-{name}")
                           for name in constant_input_names]
        ...
        for name in nodes[0].parameters:
            para = self.combine_parameters(*(node.parameters[name] for node in nodes), name=name)
