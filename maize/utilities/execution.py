...
from maize.utilities.utilities import make_list, unique_id, split_multi, set_environment
...
class _JobHandler:
    ...
    def __init__(self, counter: _JobCounterSemaphore) -> None:
...
class CommandRunnerPSIJ(ProcessBase):
    ...
    def _make_callback(
        ...
        def _callback(job: Job, status: JobStatus) -> None:
            if status.final:
                self.logger.info("Job completed (%s/%s)", count.total, max_count)
                if job.status.exit_code is not None and job.status.exit_code > 0:
                    res = self._job_to_completed_process(job)
                    self.logger.warning("Job failed with exit code %s", res.returncode)
    ...
    def run_async(
        ...
        return RunningProcess(job, verbose=verbose, raise_on_failure=self.raise_on_failure)
    ...
    def run_only(
        ...
        with _JobHandler(counter=_JobCounterSemaphore()) as hand:
    ...
    def run_parallel(
        ...
        with _JobHandler(counter=counter) as hand:
            while not queue.empty():
    ...
    def _attempt_submission(self, job: Job) -> Job:
        ...
        for i in range(self.max_retries):
            try:
                ...
                self.logger.info("Submitted job %s", job)
                return job
