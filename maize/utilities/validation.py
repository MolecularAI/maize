...
class FileValidator(Validator):
    ...
    def __init__(self, expect: Path | list[Path], zero_byte_check: bool = True) -> None:
        self.expect = make_list(expect)
        self.zero_byte_check = zero_byte_check

    def __repr__(self) -> str:
        return (f"{self.__class__.__name__}({', '.join(p.as_posix() for p in self.expect)}, "
    ...
    def __call__(self, _: subprocess.CompletedProcess[bytes]) -> bool:
        for exp in self.expect:
            if not exp.exists():
                return False
            if self.zero_byte_check and exp.stat().st_size == 0:
                return False
        return True
