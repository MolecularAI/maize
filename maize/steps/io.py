...
from maize.core.interface import Parameter, FileParameter, Input, Output, Flag
...
class LoadData(Node, Generic[T]):
    ...
    def run(self) -> None:
        ...
        if data is None:
            raise ValueError("You must specify a non-'None' value to pass on")
...
class Return(Node, Generic[T]):
    ...
    def get(self) -> T | None:
        ...
        if not self.ret_queue.empty():
            return cast(T, dill.loads(self.ret_queue.get()))
        return None
