...
from maize.utilities.utilities import Timer, unique_id
...
if TYPE_CHECKING:
    ...
    MessageType: TypeAlias = TracebackException | StatusUpdate | None
...
class Component:
    ...
    @classmethod
    def serialized_summary(cls) -> _SerialType:
        """
    ...
    def __init__(
        ...
        if parent is None:
            ...
            self.message_queue: "Queue[MessageType]" = ctx.Queue()

            # This is the logging-only queue
            self.logging_queue: "Queue[LogRecord | None]" = ctx.Queue()
            ...
            self.message_queue = parent.message_queue
            self.logging_queue = parent.logging_queue
    ...
    def as_dict(self) -> dict[str, Any]:
        ...
        data = with_fields(self, self._COMPONENT_FIELDS)
        data["parameters"] = {k: para.value for k, para in self.parameters.items() if para.is_set}
    ...
    def ports_active(self) -> bool:
        ...
        for port in self.ports.values():
            subports: list[Port[Any]] | MultiPort[Any] = (
                port if isinstance(port, MultiPort) else [port]
            )
