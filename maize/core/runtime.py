    ...
    def __exit__(self, *_: Any) -> None:
        ...
        while self._procs:
            ...
            if proc.is_alive():
                proc.terminate()
                log.debug(
                    "Terminated '%s' (PID %s) with exitcode %s", proc.name, proc.pid, proc.exitcode
                )
...
def format_update(summaries: dict[tuple[str, ...], StatusUpdate]) -> str:
    """
    ...
    for (*_, name), node_sum in summaries.items():
        color = _STATUS_COLORS[node_sum.status]
        msg += f"\n{'':>34} | {name:>16} | {color}{node_sum.status.name}{_RESET}"

        # Show queued item information, but only if the node is running
        # (otherwise this value will never be updated)
        if ((node_sum.n_inbound > 0 or node_sum.n_outbound > 0)
            and node_sum.status not in (Status.COMPLETED, Status.FAILED, Status.STOPPED)):
...
def setup_build_logging(
    ...
    stream_formatter = CustomFormat(defaults=dict(comp_name=name))
