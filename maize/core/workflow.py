...
from maize.core.interface import Input, Output, MultiParameter, MultiInput, MultiOutput
...
from maize.utilities.utilities import Timer, nested_dict_to_tuple, tuple_to_nested_dict, NestedDict
...
def expose(factory: Callable[[], "Workflow"]) -> Callable[[], None]:
    ...
    def wrapped() -> None:
        flow = factory()
        flow.description = Workflow.get_workflow_summary(name=name)
        setup_workflow(flow)
...
class Workflow(Graph, register=False):
    ...
    def from_dict(cls, data: dict[str, Any], **kwargs: Any) -> "Workflow":
        ...
        for node_data in data["nodes"]:
            if any(field not in node_data for field in ("name", "type")):
                raise ParsingException("Node specification requires at least 'name' and 'type'")

            node_type: type[Component] = Component.get_node_class(node_data["type"])
            init_data = with_keys(node_data, cls._COMPONENT_FIELDS)
        ...
        for param_data in data.get("parameters", []):
            ...
            graph.combine_parameters(*parameters_to_map, name=param_data["name"])
    ...
    def set_global_attribute(self, __name: str, __value: Any, /) -> None:
    ...
    def as_dict(self) -> dict[str, Any]:
        return with_fields(self, self._GRAPH_FIELDS)
    ...
    def add_arguments(self, parser: _A) -> _A:
        ...
        for name, param in self.all_parameters.items():
            ...
            match dtype:
                ...
                # Just a simple flag
                case _b.bool:
                    parser.add_argument(f"--{name}", action="store_true", help=param.doc)
    ...
    def update_settings_with_args(self, args: argparse.Namespace) -> None:
        ...
        """
        if args.quiet:
            self.set_global_attribute("level", logging.WARNING)
            self.logger.setLevel(logging.WARNING)
        if args.debug:
            self.set_global_attribute("level", logging.DEBUG)
            self.logger.setLevel(logging.DEBUG)
        if args.log:
            self.set_global_attribute("logfile", args.log)
        if args.keep or args.debug:
            self.set_global_attribute("cleanup_temp", False)
        if args.parameters:
    ...
    def execute(self) -> None:
        ...
        logger = Logger(message_queue=self.logging_queue, file=self.logfile)
        ...
        receive = cast(
            ...
            functools.partial(
                _get_message, message_queue=self.message_queue, timeout=TIME_STEP_SEC
            ),
        )
        ...
        with RunPool(*self.active_nodes, logger) as pool:
            ...
            while not self.signal.is_set() and len(summaries) < (pool.n_processes - 1):
                ...
                match event:
                    case StatusUpdate(status=Status.COMPLETED | Status.FAILED | Status.STOPPED):
                        ...
                        node = self.get_node(*event.parents, event.name)
                    ...
                    case StatusUpdate():
                        ...
                        if delta.seconds > (STATUS_INTERVAL_MIN * 60) and len(latest_status) > 0:
                            self.logger.info(format_update(latest_status))
                            update_time = datetime.now()
                    ...
                    case TracebackException():
                        self.to_checkpoint(fail_ok=True)
                        raise NodeException.from_traceback_exception(event)
