    ...
    def check(self) -> None:
        ...
        for node in self.flat_nodes:
            for name, param in node.parameters.items():
                ...
                # Parameters set in the config will not be set in the node._prepare
                # call, so we also exclude those.
                if (
                    not param.is_set
                    and not param.optional
                    and name not in orig_names
                    and name not in self.config.nodes[node.name].parameters
                ):
