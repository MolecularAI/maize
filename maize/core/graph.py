...
from maize.core.component import Component, SerialType, serialized_summary
...
from maize.utilities.utilities import extract_type, graph_cycles, is_path_type, matching_types, unique_id
...
class Graph(Component, register=False):
    ...
    def serialized_summary(cls) -> SerialType:
        # We only have full interface information *after* building the subgraph,
        # so we run the `build()` method (called on initialization) without
        # checking connectivity.
        item = cls(check=False)
        return serialized_summary(item, name=cls.__name__)
    ...
    def setup_directories(self, parent_path: Path | None = None) -> None:
        ...
        # If our graph directory already exists, we add a unique ID
        if self.work_dir.exists():
            self.work_dir = Path(parent_path / f"graph-{self.name}-{unique_id(6)}")
