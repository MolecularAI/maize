...
class Node(Component, Runnable, register=False):
        ...
        for name, param in self.parameters.items():
            if not param.is_set and not param.optional:
                raise NodeBuildException(
                    f"Parameter '{name}' of node '{self.name}' needs to be set explicitly"
                )
    ...
    def run_command(
        ...
        cmd = CommandRunner(
            ...
            prefer_batch=self.batch_options.is_set or batch_options is not None,
    ...
    def run_multi(
        ...
        raise_on_failure
            Whether to raise an exception when encountering a failure
        n_jobs
            Number of processes to spawn at once, should generally be
    ...
    def prepare(self) -> None:
        ...
        for exe in self.required_callables:
            ...
            if exe in (script_dic := self.node_config.scripts | self.scripts.value):
                self.runnable[exe] = " ".join(
                    expand_shell_vars(p).as_posix() for p in script_dic[exe].values()
                )

            # Prepare custom command locations
            elif exe in (com_dic := self.node_config.commands | self.commands.value):
                self.runnable[exe] = expand_shell_vars(Path(com_dic[exe])).as_posix()
    ...
    def execute(self) -> None:
        ...
        self.logger = setup_node_logging(
            name=self.name, logging_queue=self.logging_queue, level=self.level
        )
    ...
    def _attempt_loop(self) -> traceback.TracebackException | None:
        ...
        for attempt in range(self.n_attempts):
            ...
            # Error in run()
                ...
                if not self.ports_active():
                    self.logger.error("Cannot restart due to closed ports")
                    self.signal.set()
                    break
