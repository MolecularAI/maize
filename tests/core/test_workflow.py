...
class Test_graph_run:
    def test_single_execution_channel_simple(self, simple_data, example_a):
        ...
        g.execute()
        assert dill.loads(t.ret_queue.get(timeout=10)) == simple_data
    ...
    def test_single_execution_channel_normal(self, normal_data, example_a_any):
        ...
        g.connect(a.out, t.inp)
        g.execute()
        assert dill.loads(t.ret_queue.get(timeout=10)) == normal_data

    @pytest.mark.skipif(
        DEFAULT_CONTEXT == "spawn",
        reason=("Acceptable datatypes for channels are restricted "
    ...
    def test_single_execution_channel_weird(self, weird_data, example_a_any):
        ...
        g.connect(a.out, t.inp)
        g.execute()
        assert dill.loads(t.ret_queue.get(timeout=10)).keys() == weird_data.keys()

    @pytest.mark.skipif(
        not ENV_EXEC.exists(),
        reason=("Testing alternative environment execution requires "
    ...
    @pytest.mark.skip(reason="This seems to hang sometimes in VSCode")
